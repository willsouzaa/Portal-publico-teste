...
export default async function HomePage({
  ...
  <section className="container py-16 lg:py-24" style={{ marginTop: '-200px', position: 'relative', zIndex: 2 }}>
      ...
      {/* Botão WhatsApp flutuante, agressivo e com tracking */}

      <WhatsappButton />

  {/* Modal de captura de leads (Lead em 1 minuto) */}
  <LeadModalClientWrapper />
  <DebugOpenModal />

      {/* SEO: Cards de bairros e cidades cadastrados */}
      <section className="container py-16 lg:py-24">
        <div className="space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {/* Gerar cards de SEO para bairros/cidades cadastrados */}
            {Array.from(new Set(empreendimentos.map(e => `${e.cidade}, ${e.estado}`))).map((cidadeTag) => {
              // Buscar um imóvel para imagem
      ...
      {/*
        Para capturar o WhatsApp do lead antes de liberar o link, implemente um modal/popup com formulário.
        Exemplo: ao clicar, abrir modal pedindo nome/telefone, salvar lead, depois liberar o link.
        Isso exige lógica extra (React state/modal) e backend para armazenar leads.
      */}
      {/* Removido o estilo de animação para o botão do WhatsApp */}
    </>
  );
}
